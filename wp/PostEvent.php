...
class PostEvent extends PostBase {
    ...
    public function registerPost() {
        ...
        $textCategory = __('Category');
        $textCategories = __('Categories');
        register_taxonomy(PostEvent::TAXONOMY_CATEGORY, [self::TYPE], [WPTaxonomyArguments::HIERARCHICAL => true,
        ...
        $textEvent = __('Event', WpApp::TEXT_DOMAIN);
        $textEvents = __('Events', WpApp::TEXT_DOMAIN);
        register_post_type(PostEvent::TYPE, [WPostArguments::HIERARCHICAL => false,
    ...
    public function handlePostColumns($columns) {
        self::arrayInsert($columns, 2, [PostEvent::TAXONOMY_CATEGORY => __("Categories")]);
    ...
    public function registerPostMetaBoxes($meta_boxes) {
        //[MAP]
        $textEventLocation = __('Location');
        $textAddress = __('Address');
        $textAddressDescription = __('Name');
        $mapKey = get_option(SettingsSite::GOOGLE_MAP_API) ? get_option(SettingsSite::GOOGLE_MAP_API) : "";
        $siteAddress = get_option(SettingsSite::ADDRESS);
        ...
        $meta_boxes[] = [MetaBox::ID => PostEvent::META_BOX_LOCATION,
            ...
            MetaBox::FIELDS => [[MetaBoxFieldInput::ID => PostEvent::META_ADDRESS_DESCRIPTION,
