...
class ApiBooking
{
    ...
    public function postBookCheck($companyId, array $appointments)
    {
        ...
        foreach ($appointments as $appointment) {
            if (!isset($appointment['id'], $appointment['staff_id'], $appointment['datetime'])) {
                throw new Exception('Запись должна содержать все обязательные поля: id, staff_id, datetime.');
            }
        }
    ...
    )
    {
        ...
        if (!isset($person['phone'], $person['fullname'], $person['email'])) {
            throw new Exception('Клиент должен содержать все обязательные поля: phone, fullname, email.');
        }
        $parameters = array_merge($parameters, $person);
        if (!count($appointments)) {
            throw new Exception('Должна быть хотя бы одна запись.');
        }
    ...
    public function postCompany(array $fields, $userToken)
    {
        if (!isset($fields['title'])) {
            throw new Exception('Для создании компании обязательно название компании.');
        }
     ...
     * @param integer $entity - ID Объект категории (0 - общие метки, 1 - метки клиентов, 2 - метки записей)
     * @return array
     * @access public
     * @throws Exception
     * @see http://docs.yclients.apiary.io/#reference/3/0/0
     */
    public function getCompanyCategories($companyId, $entity = 0)
    {
        return $this->request('service_categories/' . $companyId . '/' . $entity);
    }
    ...
    public function postHooks($companyId, $fields, $userToken)
    {
        ...
        if (!isset($fields['active'])) {
            throw new Exception('Не передан обязательный параметр active');
        }
    ...
    protected function request($url, $parameters = [], $method = 'GET', $auth = true)
    {
        ...
        if ($auth) {
            if (!$this->tokenPartner) {
                throw new Exception('Не указан токен партнёра');
            }
